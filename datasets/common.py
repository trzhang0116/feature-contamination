...
def get_dataloader(dataset, is_train, batch_size, device, image_encoder=None, distributed=False, pin_memory=False):
    if image_encoder is not None:
        feature_dataset = FeatureDataset(is_train, image_encoder, dataset, device)
        if distributed:
            sampler = torch.utils.data.distributed.DistributedSampler(feature_dataset, shuffle=is_train)
            dataloader = DataLoader(feature_dataset, batch_size=batch_size, sampler=sampler, pin_memory=pin_memory, prefetch_factor=2)
        else:
            sampler = None
            dataloader = DataLoader(feature_dataset, batch_size=batch_size, shuffle=is_train, pin_memory=pin_memory, prefetch_factor=2)
    else:
        sampler = dataset.train_sampler if is_train else dataset.test_sampler
